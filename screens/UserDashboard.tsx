...
export const UserDashboard: React.FC<Props> = ({ user, onUpdateUser, onReturnToAdmin }) => {
  ...
  const handlePlanAction = async (action: 'pause' | 'reschedule' | 'restart') => { if (!currentPlan) return; const config = user.planConfigs[currentPlan.id] || { startDate: getTodayStr(), isPaused: false }; if (action === 'restart') { const planGoalIds = new Set<string>(); currentPlan.disciplines.forEach(d => { d.subjects.forEach(s => { s.goals.forEach(g => planGoalIds.add(g.id)); }); }); const currentCompleted = (user.progress.completedGoalIds as string[]) || []; const newCompleted = currentCompleted.filter(id => !planGoalIds.has(id.split(':')[0])); const newRevisions = (user.revisions || []).filter(r => !planGoalIds.has(r.sourceGoalId)); const updatedUser = { ...user, progress: { ...user.progress, completedGoalIds: newCompleted, planStudySeconds: { ...user.progress.planStudySeconds, [currentPlan.id]: 0 } }, revisions: newRevisions, planConfigs: { ...user.planConfigs, [currentPlan.id]: { startDate: getTodayStr(), isPaused: false } } }; onUpdateUser(updatedUser); await saveUserToDB(updatedUser); return; } let newConfig = { ...config }; if (action === 'pause') newConfig.isPaused = !newConfig.isPaused; else if (action === 'reschedule') { newConfig.startDate = getTodayStr(); newConfig.isPaused = false; } const updatedUser = { ...user, planConfigs: { ...user.planConfigs, [currentPlan.id]: newConfig } }; onUpdateUser(updatedUser); await saveUserToDB(updatedUser); };
  ...
  const renderEditalView = () => {
    ...
    currentPlan.editalVerticalizado.forEach(disc => { disc.topics.forEach(topic => { totalTopics++; if (isTopicDone(topic)) completedTopics++; if (topic.subTopics) { topic.subTopics.forEach(st => { totalTopics++; if (isSubTopicDone(st)) completedTopics++; }); } }); });
    ...
    const renderLinksRow = (links: any) => (
        <div className="flex flex-wrap gap-2 ml-7 mt-1">
            {ORDERED_LINKS.map(type => {
                ...
                const personalMindMaps = user.personalMindMaps?.[goal.id] || []; 
                ...
                if (type === 'resumo') {
                    ...
                    if (goal.generatedMindMap) resources.push({ type: 'mindmap', data: goal.generatedMindMap, label: 'Mapa Mental (IA)' });
                        ...
                        <div key={type} className={`flex flex-col transition-all duration-200 ${isExpanded ? 'w-full my-2 bg-[#151515] rounded-xl border border-[#333] p-4' : ''}`}>
                             <div className="flex items-center">
                                ...
                                <button onClick={() => toggleEditalSubGoals(goal.id)} className={`flex items-center gap-2 px-2 py-1 rounded border text-[10px] font-bold uppercase transition hover:brightness-125 w-fit ${isGoalDone ? '!border-green-500 !bg-green-500/10 !text-green-500' : ''}`} style={{ borderColor: isGoalDone ? undefined : goal.color || '#333', color: isGoalDone ? undefined : goal.color || '#999' }}>
                                    <IconComp className="w-3 h-3"/> {goal.title} 
                                    {revCount && <span className="text-[8px] bg-cyan-900/40 text-cyan-400 px-1 rounded ml-1 border border-cyan-800">{revCount}</span>}
                                    <span className="opacity-60 font-mono text-[9px] ml-1">({resources.length} itens + {personalMindMaps.length} mapas)</span> <Icon.ChevronDown className={`w-3 h-3 transition-transform ${isExpanded ? 'rotate-180' : ''}`}/>
                                </button>
                            </div>
                            ...
                            {isExpanded && ( 
                                <div className="mt-4 space-y-6 animate-fade-in pl-1"> 
                                             ...
                                             <div className="flex flex-col gap-2">
                                                {resources.map((res, idx) => ( 
                                                    <button key={idx} onClick={() => { 
                                                        ...
                                                        else if (res.type === 'mindmap') setActiveMindMapNode(res.data!);
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center border-b border-[#333] pb-1">
                                            <h4 className="text-[9px] font-black text-purple-500 uppercase tracking-widest">Meus Mapas Mentais ({personalMindMaps.length}/5)</h4>
                                            {personalMindMaps.length < 5 && (
                                                <button onClick={() => handlePersonalMindMapAction(goal.id, 'add')} className="text-[8px] font-black text-white hover:text-purple-400 transition uppercase">+ Criar Novo Mapa</button>
                                            )}
                                        </div>
                                        
                                        <div className="grid gap-3">
                                            {personalMindMaps.map(map => (
                                                <div key={map.id} className="bg-black/40 border border-purple-500/30 rounded-xl p-3 flex items-center justify-between hover:border-purple-500/60 transition group">
                                                    <div className="flex items-center gap-3 overflow-hidden flex-1">
                                                        <div className="w-8 h-8 rounded-lg bg-purple-900/20 flex items-center justify-center border border-purple-500/20 text-purple-400">
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            {editingMindMapNameId === map.id ? (
                                                                <input 
                                                                    autoFocus 
                                                                    value={map.name} 
                                                                    onBlur={() => setEditingMindMapNameId(null)}
                                                                    onChange={e => handlePersonalMindMapAction(goal.id, 'update', { ...map, name: e.target.value })}
                                                                    className="bg-black border border-purple-500/50 rounded px-2 py-0.5 text-xs text-white outline-none w-full"
                                                                />
                                                            ) : (
                                                                <div className="flex items-center gap-2">
                                                                    <h5 className="font-bold text-gray-200 text-xs truncate">{map.name}</h5>
                                                                    <button onClick={() => setEditingMindMapNameId(map.id)} className="text-gray-600 hover:text-white opacity-0 group-hover:opacity-100 transition"><Icon.Edit className="w-3 h-3"/></button>
                                                                </div>
                                                            )}
                                                            <p className="text-[8px] text-gray-500 font-mono">Criado em: {new Date(map.createdAt).toLocaleDateString()}</p>
                                                        </div>
                                                    </div>
                                                    <div className="flex gap-2 items-center">
                                                        <button onClick={() => setMigrationData({ type: 'mindmap', item: map })} className="text-gray-600 hover:text-white transition p-1" title="Copiar para outro plano"><Icon.Copy className="w-3.5 h-3.5"/></button>
                                                        <button onClick={() => setActiveMindMapNode(map.root)} className="text-gray-600 hover:text-purple-400 transition p-1" title="Visualizar">
                                                            <Icon.Eye className="w-3.5 h-3.5"/>
                                                        </button>
                                                        <button onClick={() => setActivePersonalMindMap(map)} className="px-3 py-1.5 bg-purple-600/20 text-purple-300 border border-purple-500/30 rounded text-[9px] font-bold uppercase hover:bg-purple-600/40 transition">Abrir Editor</button>
                                                        <button onClick={() => handlePersonalMindMapAction(goal.id, 'delete', map)} className="text-gray-600 hover:text-red-500 transition p-1"><Icon.Trash className="w-3.5 h-3.5"/></button>
                                                    </div>
                                                </div>
                                            ))}
                                            {personalMindMaps.length === 0 && (
                                                <p className="text-[9px] text-gray-600 italic text-center py-4 border border-dashed border-[#333] rounded-lg">Você ainda não criou nenhum mapa mental para este assunto.</p>
                                            )}
                                        </div>
                                    </div>
                                </div> 
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#050505] text-gray-200">
        ...
        {/* Visual Mind Map Editor for User Created Maps */}
        {activePersonalMindMap && (
            <VisualMindMapModal 
                rootNode={activePersonalMindMap.root} 
                onSave={(newRoot) => {
                    let foundGoalId = '';
                    if (user.personalMindMaps) {
                        for (const [gid, maps] of Object.entries(user.personalMindMaps)) {
                            if ((maps as PersonalMindMap[]).some(m => m.id === activePersonalMindMap.id)) {
                                foundGoalId = gid;
                                break;
                            }
                        }
                    ...
                    if (foundGoalId) {
                        handlePersonalMindMapAction(foundGoalId, 'update', { ...activePersonalMindMap, root: newRoot });
                    }
                }}
                onClose={() => setActivePersonalMindMap(null)}
                title={`Editando: ${activePersonalMindMap.name}`}
            />
        )}
        
        {/* Read-Only Viewer for Admin Generated Maps and User Views */}
        {activeMindMapNode && !activePersonalMindMap && (
            <MindMapModal 
                rootNode={activeMindMapNode} 
                onClose={() => setActiveMindMapNode(null)} 
            />
        )}
